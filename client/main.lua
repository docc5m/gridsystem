        ...
        for i = 1, #MarkersToCheck do
            ...
            if distance < zone.drawDistance then
                ...
                if zone.show3D then
                    ...
                    if zone.type ~= -1 then
                        DrawMarker(zone.type, zone.pos, 0.0, 0.0, 0.0, 90.0, 0.0, 0.0, zone.scale.x, zone.scale.y, zone.scale.z, zone.color.r, zone.color.g, zone.color.b, 100, false, true, 2, true, zone.textureDict, zone.textureName, false)
                    end
                end
...
RegisterNetEvent('ToggleUI', function (toggle, logo, control, text)
    SendNUIMessage({
        toggle = toggle,
        logo = logo,
        control = control,
